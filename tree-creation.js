...
var running_time = document.getElementById('running-time');
  ...
  while (!newNode) {
    if (n <= walker.data) {
      if (walker.children.length === 0) {
        ...
        // Already have right child, left child is empty
...
function deleteTree(n, callback) {
  ...
  if (n === walker.data) {
    ...
    // Tree only has root node
      ...
      if (nodeDelete.children[0].data === null) {
        // Root does not have left subtree
  ...
  if (nodeDelete.children.length === 0) {
    ...
    // Node to be deleted is internal node
    ...
    if (!nodeReplace) {
      // No left child, right child of nodeDelete replace its position
      parent = nodeDelete.parent;
      nodeBalance = parent; // Will start balacing from this node
      ...
      // Left child of parent
        ...
        parent.children[1] = nodeDelete.children[1]; // Right child of parent
    } else {
      ...
      // Left child of parent
        ...
        parent.children[1] = nodeReplace; // Right child of parent
